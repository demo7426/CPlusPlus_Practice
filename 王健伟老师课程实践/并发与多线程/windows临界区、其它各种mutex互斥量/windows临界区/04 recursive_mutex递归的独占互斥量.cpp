...
//#else
//				std::lock_guard<std::recursive_mutex> mAguard(m_mutex);//只能加锁、解锁，在开始定义时就lock()了，作用域结束时unlock()（这里花括号就是它的作用域）;
//				std::lock_guard<std::recursive_mutex> mAguard2(m_mutex);
//				std::lock_guard<std::recursive_mutex> mAguard3(m_mutex);
//				std::lock_guard<std::recursive_mutex> mAguard4(m_mutex);
//				m_ListCon.push_back(i);
//#endif // _WINDOWCS
//
//
...
//	std::recursive_mutex m_mutex;//m_mutex为互斥量
//#ifdef _WINDOWCS
