...
int main()
{
    ...
    /*我们可以额外向std::async传递一个参数，该参数类型是std::launch类型（枚举类型），来达到一些特殊目的
        ...
        b)std::launch::async表示在调用std::async时立即执行（如果后面不写第一个参数，就默认为std::launch::async参数）*/
    ...
    std::future_status mStatus = mResult.wait_for(std::chrono::seconds(1));//wait_for()进程等待线程一段时间

    //可以用future_status判断是否成功创建异步（新线程）任务
    if (mStatus == std::future_status::timeout)
    {
        std::cout << "线程还没有执行完毕！" << std::endl;
    }
    else if (mStatus == std::future_status::ready)
    {
        std::cout << "线程已经执行完毕！并且返回值。" << std::endl;
        std::cout << mResult.get() << std::endl;//卡在这里等待MyThreadFunc()返回一个值，拿到值之后才会继续执行
    }
    else if (mStatus == std::future_status::deferred)
    {
        std::cout << "未创建新线程，线程被延迟执行！" << std::endl;
        std::cout << mResult.get() << std::endl;//卡在这里等待MyThreadFunc()返回一个值，拿到值之后才会继续执行
    }
